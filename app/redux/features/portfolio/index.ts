...
export const getAllportfolio = createAsyncThunk(
  ...
  async ({ participantId }: { participantId: string }, { rejectWithValue }) => {
    ...
    try {
      ...
      return await axios
        .get("http://127.0.0.1:8000/api/headers", config)
        .then((res) => res.data);
    } catch (error) {
...
export const getByIdportfolio = createAsyncThunk(
  ...
  async ({ participantId }: { participantId: string }, { rejectWithValue }) => {
    ...
    try {
      return await axios
        .get(
          "http://127.0.0.1:8000/api/headers/67ab65b24cb48a7c886d0dfa",
          config
        )
        .then((res) => res.data);
...
export const getByIdgallery = createAsyncThunk(
  ...
  async ({ participantId }: { participantId: string }, { rejectWithValue }) => {
    ...
    try {
      return await axios
        .get(
          "http://127.0.0.1:8000/api/galleries/67aecc532071993d23e91175",
          config
        )
        .then((res) => res.data);
